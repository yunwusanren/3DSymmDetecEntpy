...
void mesh_cpca_covariance(TriMesh *mesh, float C[3][3])
{
	if (mesh->faces.empty() && mesh->tstrips.empty()) {
		point_covariance(mesh->vertices, C);
		return;
	...
	point m = mesh_center_of_mass(mesh);
	...
	for (int i = 0; i < n; i++) {
		...
		for (int v = 0; v < 3; v++) {			
			...
			for (int j = 0; j < 3; j++)
				for (int k = j; k < 3; k++)
					C[j][k] += area[i] * pm[j] * pm[k];
		}
			
		// Covariance of centroid
		point c = (p[f[0]] + p[f[1]] + p[f[2]]) / 3.0f-m;
		for (int j = 0; j < 3; j++)
			for (int k = j; k < 3; k++)
				C[j][k] += 9*area[i] * c[j] * c[k];
	}

	for (int j = 0; j < 3; j++)
		for (int k = j; k < 3; k++)
			C[j][k] = C[j][k]/(12*totarea);
...
// Compute cpca covariance of faces (integral area-weighted) in a mesh
void mesh_cpca_part_covariance(TriMesh *mesh, int axis, float C[3][3],float C_[3][3])
{
	if (mesh->faces.empty() && mesh->tstrips.empty()) {
		point_covariance(mesh->vertices, C);
		return;
	...
	for (int i = 0; i < n; i++) {
		...
		point pm2 = p[f[1]] - m;
		point pm3 = p[f[2]] - m;
		//if(pm1[axis]>0 && pm2[axis]>0 && pm3[axis]>0)  //x/y/z>0 
		//if(pm1[axis]>0 || pm2[axis]>0 || pm3[axis]>0)  //x/y/z>0 
		{
			for (int v = 0; v < 3; v++) 
			{			
	...
	for (int j = 0; j < 3; j++)
		for (int k = j; k < 3; k++)
			C_[j][k] = C_[j][k]/(12*totarea);

	C_[1][0] = C_[0][1];
	C_[2][0] = C_[0][2];
	C_[2][1] = C_[1][2];
}
// Compute npca covariance of faces (integral area-weighted) in a mesh
void mesh_npca_covariance(TriMesh *mesh, float C[3][3])
{
	if (mesh->faces.empty() && mesh->tstrips.empty()) {
		point_covariance(mesh->vertices, C);
		return;
	...
	// Compute area and normals of each face as well as  the total area
	float totarea = 0.0f;
	float *area;
	vec* facenormal;
	area = new float[n];	
	facenormal= new vec[n];
	const vector<point> &p = mesh->vertices;	
       #pragma omp parallel for	
	for (int i = 0; i < n; i++) 
	{
		...
		const point &p0 =p[f[0]];
		const point &p1 =p[f[1]];
		const point &p2 =p[f[2]];	
		vec a = p1-p0, b = p2-p1;
		facenormal[i] = a CROSS b;	
		normalize(facenormal[i]);
	}
	...
	for (int i = 0; i < n; i++) {			
			for (int j = 0; j < 3; j++)
				for (int k = j; k < 3; k++)
					C[j][k] += area[i] * facenormal[i][j] * facenormal[i][k];			
	}
