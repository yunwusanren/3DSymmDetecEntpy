...
static PSBCategory* defineCategory(const char *categoryName, const char *fullName, int numModels){
  ...
  //strcpy(temp1,categoryName); 
  //category->_name=temp1;//strcpy(category->_name,categoryName);  // category->_name =(char*)categoryName;// strdup(categoryName);
  category->_name =_strdup(categoryName);
  assert(category->_name != NULL);
  //strcpy(temp2,fullName); 
  //category->_fullName=temp2;
  //strcpy(category->_fullName,fullName);  //category->_fullName =(char*)fullName;// strdup(fullName);
